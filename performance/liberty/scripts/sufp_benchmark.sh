...
IBM_JAVA_OPTIONS    - IBM Java options
...
"
    ...
    if [ -z "$IBM_JAVA_OPTIONS" ]; then
        echo "IBM JAVA Options not set."
    else
        export IBM_JAVA_OPTIONS=${IBM_JAVA_OPTIONS}
    fi
...
"
  case $PLATFORM in
    CYGWIN)
      ...
      NET STOP "IBM Cognos Content Database"
      ;;
    Linux)
      echo "Stopping Cognos database service"
...
# Print PMA header for script identification
printPMAHeader

# Print Startup and Footprint header with script version
echo "Startup and Footprint Benchmark Launch Script for WebSphere Liberty
Version: Clean-03/02/2018
###########################################################################
...
if [ $SCENARIO = "Cognos" ]; then
  case $PLATFORM in
    CYGWIN)
      ...
      NET START "IBM Cognos Content Database"
      ;;
    Linux)
      echo "Starting Cognos database service"
