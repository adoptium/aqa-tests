...
public class Workload 
{
	...
	// Prevents multiple threads from executing kill() or shutdown(). Each method will only execute on the first call.   
	private final AtomicMutex shutdownMutex;
	
	// The builders track quota for each type of payload and are organized into sets. 
	// sets[] is a flat array of all sets containing payloads, each including only payloads located directly within.   
	private PayloadBuilderSet[] sets;
	
	// A (convenience) flat list of all builders
	private PayloadBuilder[] builders; 
		
	// Allocation threads. Limited queue size and limited number of threads. Limits set via configuration. 
	...
	/**
	 * Constructs a new instance including the configuration, 
	 * container, thread pools and builders. 
	 * 
	...
	{						
		...
		// Create PayloadBuilderSets from configuration by traversing <payloadSet ...> tags 
		traverseSets(); 
	...
	/**
	 * Starts running the workload. Returns as soon as the 
	 * workload is running. 
	 */
	 ...
	 * 
	 * @throws InterruptedException - thrown if any of the join() calls are interrupted. 
	 */
	...
	/**
	 * Shutdown immediately using as little heap as possible. Doesn't 
	 * block on thread termination.  
	 */
	protected void kill()
	{
		synchronized (this) // Don't allow shutdown during startup. 
	...
	/**
	 * Creates and adds new builder sets for all sets containing payloads in the tree of 
	 * sets. Adds all builders to a mater builder list. Uses a BFS to traverse the tree.  
	 * 
	 * 
	 */
	...
	{	
		...
		{			
			...
			while (!queue.isEmpty())
			{
				...
				for (PayloadSetConfiguration childSetConfiguration : setConfiguration.getChildPayloadSetConfigurations())
	...
	/**
	 * Returns a formatted representation of the configuration data encapsulated in this 
	 * Workload.  	
	 * 
	...
	public String toString()
	{
		...
		try 
		{
			...
			table.addHeadings(table.new Headings(
				new String[] {"WORKLOAD", workloadConfiguration.getID() + ""}, 
				new int[] {24, 20}, 
				alignment, true, false
			));
			table.addRow(table.new Row(new String[] {"Start Time", Utilities.formatTime(workloadConfiguration.getStartTime())}, alignment));
			table.addRow(table.new Row(new String[] {"Duration", Utilities.formatTime(workloadConfiguration.getDuration())}, alignment));						
			table.addRow(table.new Row(new String[] {"Producer threads", workloadConfiguration.getNumProducerThreads() + ""}, alignment));			
			table.addRow(table.new Row(new String[] {"Maintenance period", Utilities.formatTime(workloadConfiguration.getMaintenancePeriod())}, alignment));				 
			table.addRow(table.new Row(new String[] {"Pay period", Utilities.formatTime(workloadConfiguration.getPaymentPeriod())}, alignment));
			table.addLine(); 
	...
	/**
	 * Gets the average allocation data rate since the 
	 * start of this workload 
	 * 
	...
	/**
	 * Gets the throughput in bytes up 'till this point, as reported by the thread pool.  
	 * 
	 * @return
	 */
	...
	/**
	 * Gets an estimate of the total number of bytes of throughput up until this point. 
	 * 
	 * @return
	 */
	protected long getLiveSetSize()
	{
		...
		/* 
		 * The total number of butes still alive is the throughput 
		 * minus the number of bytes either removed from the 
		 * container or never added in the first place.  
		 */
	...
	{									
		/**
		 * Constructs a new PayerThread for the given WorkloadConfiguration and builders.  
		 * 
		 * @param workloadConfiguration
		 * @param builders
		 */
