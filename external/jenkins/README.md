...
0. Ensure your test machine is set up with [test prereqs]`(https://github.com/adoptium/oaqa-tests/blob/master/docs/pages/Prerequisites.md)`. For external tests, you do need Docker installed.
...
When [running these from the command-line]`(https://github.com/adoptium/aqa-tests/blob/master/docs/userGuide`.md#local-testing-via-make-targets-on-the-commandline), these tests are grouped under a make target called 'external', so 'make external' would run the entire set of tests found in the aqa-tests/external directory. This is unadvisable! Limit what you compile and run, BUILD_LIST=external/`<someSubDirectory>`, and TARGET=`<testCaseNameFromSubdirPlaylist>`
