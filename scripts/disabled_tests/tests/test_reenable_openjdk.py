...
from reenable_openjdk import (
    parse_reenable_command,
    parse_optional_params,
    resolve_targets_from_params,
    get_all_jdk_versions,
    remove_exclusion_from_file,
    ReenableCommandError,
)
...
class TestParseReenableCommand(TestCase):
    """Test parsing of /re-enable command."""
    
    def test_simple_command(self):
        comment = "/re-enable test.java macosx-all"
        testcases, platform, params = parse_reenable_command(comment)
    ...
    def test_command_with_jdk_param(self):
        comment = "/re-enable test.java macosx-all jdk=11,17,21"
        testcases, platform, params = parse_reenable_command(comment)
    ...
    def test_command_with_impl_param(self):
        comment = "/re-enable test.java linux-x64 jdk=11,21 impl=openj9"
        testcases, platform, params = parse_reenable_command(comment)
    ...
    def test_command_in_multiline_comment(self):
        comment = """
Some text before
/re-enable test.java macosx-all jdk=11
Some text after
"""
        testcases, platform, params = parse_reenable_command(comment)
    ...
    def test_invalid_command_missing_platform(self):
        comment = "/re-enable test.java"
        
        with self.assertRaises(ReenableCommandError) as cm:
            parse_reenable_command(comment)
        
        self.assertIn("Invalid format", str(cm.exception))
    
    def test_invalid_command_no_reenable(self):
        comment = "some other comment"
        
        with self.assertRaises(ReenableCommandError) as cm:
            parse_reenable_command(comment)
        
        self.assertIn("No /re-enable command found", str(cm.exception))
...
class TestRemoveExclusionFromFile(TestCase):
    """Test removing exclusion from file."""
...
""")
    ...
    def test_remove_existing_exclusion(self):
        result = remove_exclusion_from_file(self.temp_file.name, 'test1.java', 'macosx-all')
        ...
        self.assertNotIn('test1.java', content)
        # Other entries should remain
        self.assertIn('test2.java', content)
        self.assertIn('test3.java', content)
    
    def test_remove_nonexistent_exclusion(self):
        result = remove_exclusion_from_file(self.temp_file.name, 'test4.java', 'macosx-all')
    ...
    def test_remove_wrong_platform(self):
        # test1.java exists but only for macosx-all, not linux-x64
        result = remove_exclusion_from_file(self.temp_file.name, 'test1.java', 'linux-x64')
        self.assertFalse(result)
        
        # File should be unchanged
        with open(self.temp_file.name, 'r') as f:
            content = f.read()
        self.assertIn('test1.java', content)
    
    def test_comments_are_preserved(self):
        remove_exclusion_from_file(self.temp_file.name, 'test1.java', 'macosx-all')
        
        with open(self.temp_file.name, 'r') as f:
            content = f.read()
        
        self.assertIn('# Test ProblemList file', content)
        self.assertIn('# Comment line', content)
    
    def test_remove_middle_entry(self):
        result = remove_exclusion_from_file(self.temp_file.name, 'test2.java', 'linux-x64')
        self.assertTrue(result)
        
        with open(self.temp_file.name, 'r') as f:
            content = f.read()
        
        self.assertNotIn('test2.java', content)
        self.assertIn('test1.java', content)
        self.assertIn('test3.java', content)
